...
const fmtPrice = (amount: number, countrySlug: string) => formatPrice(amount, countrySlug);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (activeCountry && pathname === '/allproduct') {
          router.replace(`/country/${activeCountry}/allproduct`);
        }
      } catch (e) {}
    }
  }, [pathname, router]);
                  ...
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div>
                      ...
                      <p className="text-[.84rem] text-[#7a6b5e]">
                        ...
                        {" "}products available
                        {" · prices in "}
                        <span className="font-semibold text-[#b5623b]">{formatPrice(0, currency).split('0')[0]} {currency.code}</span>
                      </p>
                    </div>
                ...
                {allProducts.length === 0 && !loading && !error ? (
                  <div className="flex flex-col items-center justify-center min-h-[320px] gap-4 text-center">
                    {/* <div className="text-5xl">{currency.flag}</div> */}
                     <div className="text-[1rem] font-serif font-semibold text-[#1e1610]">
