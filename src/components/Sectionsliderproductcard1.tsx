...
// Unsplash flower images per colour (fallback to gradient if fails)
...
// Circle background colour per colour name
const COLOR_CIRCLE_BG: Record<string, string> = {
  red:      '#e85d5d',
  purple:   '#7c5cbf',
  pink:     '#e87ab4',
  peach:    '#f5c49a',
                ...
                <button onClick={() => addToCart(qty)} disabled={outOfStock || adding}
                  ...
                  {adding ? <><svg className="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" /></svg>Adding…</>
                    : added ? <><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>Added!</>
                      : <><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>{outOfStock ? 'Out of Stock' : 'Add to Cart'}</>}
                </button>
              </div>
            </div>
            ...
            {loading ? <><svg className="h-3.5 w-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" /></svg>Adding…</>
              : added ? <><svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>Added!</>
                : outOfStock ? 'Out of Stock'
                  : <><svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>Add to Cart</>}
          </button>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// ★ ColourPickerSection — Image 1 style
//   Clicking a colour → router.push('/shop?color=red')
//   Change '/shop' below to whatever your AllProductsPage route is
// ─────────────────────────────────────────────────────────────────────────────

const PRODUCTS_PAGE_ROUTE = '/allproduct' // ← adjust to your actual route e.g. '/products', '/flowers'
    ...
    <div className="relative w-full rounded-3xl overflow-hidden py-10 px-6 mb-10"
      ...
      <div className="flex flex-wrap justify-center gap-x-6 gap-y-8 max-w-5xl mx-auto">
        {colors.map((color, idx) => {
          ...
          const circleBg = COLOR_CIRCLE_BG[color.name.toLowerCase()] ?? color.hex
          ...
          return (
            <button
              ...
              onClick={() => router.push(`${PRODUCTS_PAGE_ROUTE}?color=${encodeURIComponent(color.name.toLowerCase())}`)}
...
const SectionSliderProductCard1: FC<SectionSliderProductCardProps> = ({
  className = '',
  ...
  // Derive unique colours from products
  ...
  return (
    <>
      ...
      <div className={`nc-SectionSliderProductCard ${className}`}>
        ...
        {/* ── Colour Picker (navigates to /shop?color=xxx) ── */}
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center rounded-3xl border border-red-100 bg-red-50 py-14 text-center">
            ...
            <button onClick={() => fetchProducts(false)} className="mt-4 rounded-full bg-gray-900 px-5 py-2 text-xs font-semibold text-white transition hover:bg-gray-700">Retry</button>
          </div>
        )}
...
export default SectionSliderProductCard1