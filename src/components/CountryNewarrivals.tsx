...
export interface TApiProduct {
  ...
  // Accept any casing / shape the backend might send
...
// ─────────────────────────────────────────────────────────────────────────────
// Fetch the "New Arrivals" _id dynamically from /api/allFeaturedProducts/
// ─────────────────────────────────────────────────────────────────────────────

async function fetchNewArrivalsId(): Promise<string | null> {
  try {
    const res = await fetch(`${BASE_URL}/api/allFeaturedProducts/`, { cache: 'no-store' })
    if (!res.ok) return null
    const json = await res.json()
    // Handle both shapes: array at root OR { data: [...] }
    const list: TFeaturedProduct[] = Array.isArray(json) ? json : (json.data ?? [])
    console.log('[NewArrivals] allFeaturedProducts →', list)
    const match = list.find(
      (fp) => fp.name?.trim().toLowerCase() === NEW_ARRIVALS_LABEL.toLowerCase()
    )
    console.log('[NewArrivals] matched featuredProduct →', match)
    return match?._id ?? null
  } catch (e) {
    console.error('[NewArrivals] fetchNewArrivalsId error:', e)
    return null
  }
}
...
// Extracts whatever value lives under any known key variant for featuredProduct,
// normalises it to an array of { _id, name } objects or plain ID strings,
// then checks if any entry matches the resolved New Arrivals ID or label.
// ─────────────────────────────────────────────────────────────────────────────

function getFeaturedProductRaw(product: TApiProduct): unknown {
  // Try every possible key the backend might use
  return (
...
function matchesNewArrivals(fp: unknown, newArrivalsId: string): boolean {
  if (!fp) return false

  // Normalise to array so we handle both single-value and array cases
  const arr: unknown[] = Array.isArray(fp) ? fp : [fp]

  return arr.some((item) => {
    if (!item) return false
    // Plain string ID
    if (typeof item === 'string') {
      return item === newArrivalsId
    }
    // Populated object
    if (typeof item === 'object') {
      const obj = item as Record<string, any>
      if (obj._id === newArrivalsId) return true
      if (obj.name?.trim().toLowerCase() === NEW_ARRIVALS_LABEL.toLowerCase()) return true
    }
    return false
  })
}
    ...
    <div className="group flex flex-col">
      ...
      <div className="mt-2.5 px-0.5">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs sm:text-sm font-bold text-emerald-700">{formatPrice(data.discountPrice)}</span>
...
const CountryNewarrivals: FC<SectionSliderProductCardProps> = ({
  className = '',
  ...
  // null = ID not yet resolved | string = resolved (may be empty if not found)
  const [newArrivalsId, setNewArrivalsId] = useState<string | null>(null)
  const isMounted = useRef(true)

  const [emblaRef, emblaApi] = useEmblaCarousel({ ...emblaOptions, align: 'start', dragFree: false })
  const { prevBtnDisabled, nextBtnDisabled, onPrevButtonClick, onNextButtonClick } = useCarouselArrowButtons(emblaApi)

  // ── Step 1: resolve ID on mount ────────────────────────────────────────────
  useEffect(() => {
    isMounted.current = true
    fetchNewArrivalsId().then((id) => {
      if (isMounted.current) setNewArrivalsId(id ?? '')
    })
    return () => { isMounted.current = false }
  }, [])

  // ── Step 2: fetch products once ID is resolved ─────────────────────────────
  const fetchProducts = useCallback(async (isBackground = false) => {
    try {
      ...
      const json = await res.json()
      const all: TApiProduct[] = Array.isArray(json) ? json : (json.data ?? [])

      // ── DEBUG: log the first product so you can see the exact field shape ──
      if (all.length > 0) {
        console.log('[NewArrivals] sample product keys:', Object.keys(all[0]))
        console.log('[NewArrivals] sample featuredProduct field:', getFeaturedProductRaw(all[0]))
        console.log('[NewArrivals] newArrivalsId being used:', newArrivalsId)
      }

      const filtered = newArrivalsId
        ? all.filter((p) => matchesNewArrivals(getFeaturedProductRaw(p), newArrivalsId))
        : []

      console.log(`[NewArrivals] total: ${all.length}, filtered (New Arrivals): ${filtered.length}`)
  ...
  useEffect(() => {
    if (newArrivalsId !== null) fetchProducts(false)
  }, [fetchProducts, newArrivalsId])

  useEffect(() => {
    if (!pollInterval || newArrivalsId === null) return
    const id = setInterval(() => fetchProducts(true), pollInterval)
    return () => clearInterval(id)
  }, [fetchProducts, pollInterval, newArrivalsId])
      ...
      <div className={`nc-SectionSliderProductCard ${className}`}>
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center rounded-2xl sm:rounded-3xl border border-red-100 bg-red-50 py-12 text-center px-4">
            ...
            <button onClick={() => fetchProducts(false)} className="mt-4 rounded-full bg-gray-900 px-5 py-2 text-xs font-semibold text-white transition hover:bg-gray-700 active:scale-95">
              Retry
            </button>
          </div>
        )}
          ...
          <div className="embla overflow-hidden" ref={emblaRef}>
            <div className="embla__container flex -ml-3 sm:-ml-5">
              {products.map((product) => (
                <div key={product._id} className="embla__slide flex-[0_0_calc(100%-6px)] sm:flex-[0_0_calc(50%-10px)] md:flex-[0_0_calc(33.333%-14px)] xl:flex-[0_0_calc(25%-15px)] pl-3 sm:pl-5 min-w-0">
