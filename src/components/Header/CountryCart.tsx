...
function VendorPopup({ onClose }: { onClose: () => void }) {
  ...
  const validateStep2 = (): boolean => {
    ...
    else if (!/^[\+]?[(]?[0-9]{1,4}[)]?[-\s\.]?[(]?[0-9]{1,4}[)]?[-\s\.]?[0-9]{3,4}[-\s\.]?[0-9]{3,4}$/.test(form.phone)) {
            ...
            <>
              ...
              <div className="px-7 py-5 flex flex-col gap-4">
                {step === 1 ? (
                  ...
                  <>
                    {showBanner && (fieldErrors.email || fieldErrors.phone || fieldErrors.shopLicence || fieldErrors.general) && (
                      <ValidationBanner
                        errors={{ email: fieldErrors.email, phone: fieldErrors.phone, shopLicence: fieldErrors.shopLicence, general: fieldErrors.general }}
                        onClose={() => {
                          setFieldErrors((p) => { 
                            const n = {...p}; 
                            delete n.email; 
                            delete n.phone; 
                            delete n.shopLicence; 
                            delete n.general; 
                            return n 
                          })
                          setShowBanner(false)
                    ...
                    <Field label="Shop Licence Number" optional error={fieldErrors.shopLicence}>
                      <input className={fieldErrors.shopLicence ? inputErrCls : inputCls} 
...
export default function CountryCart() {
  ...
  const countryName = params?.name as string | undefined
